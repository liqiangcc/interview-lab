...
const { initialJournal, initialIntent, applyOne, receiptObject, receiptBody, updateJournal } = require('../scripts/lib/issue-1658-materialization-runner');
const {
...
const inputPlan = JSON.parse(fs.readFileSync('/tmp/materialization-13-plan.bound.json', 'utf8'));
const sourceSnapshot = JSON.parse(fs.readFileSync(path.join(__dirname, '../data/pilot/issue-1658/source-note-live.snapshot.json'), 'utf8'));

function freshInputs() {
  const selected = new Set(inputPlan.rows.map((row) => Number(row.request.source_note_issue_number)));
  const freshRows = new Map(sourceSnapshot.issues.filter((issue) => selected.has(Number(issue.number))).map((issue) => [Number(issue.number), { issue, comments: [] }]));
