...
test('v2 SourceFragment frontier cannot regress by manifest order', () => {
  const comments = [comment(1, record(2, 'literal')), comment(2, record(1, 'classification'))];
  const result = validate(comments);
...
test('v2 session cannot silently switch SourceSequenceManifest digest', () => {
  const second = record(2, 'classification');
  second.source_manifest_sha256 = '0'.repeat(64);
  const comments = [comment(1, record(1, 'literal')), comment(2, second)];
  const result = validate(comments);
  assert.equal(result.ok, false);
