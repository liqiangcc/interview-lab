...
Learning mode 的默认目标是**降低理解摩擦，同时保留详细、可学习的推导过程**。第一次揭示真实 interviewer cue 后，AI 应直接基于当前已经允许使用的信息进入分层分析，不要求学习者先回答、猜测或声明“不知道”。
...
AI 主动逐步解释，并严格遵守 no-look-ahead。默认优化目标是：**高质量分析 + 详细可见推导 + 最低理解摩擦 + 与已知上下文逻辑连续**。
...
用户不需要先证明自己会，也不需要完成额外任务才能继续。AI 应主动承担信息整理和概念桥接成本，但**不能用“认知压缩”删掉决定结论的关键推导步骤**。

用户主要学习的是这条可观察推导路径，而不是模型内部私有 chain-of-thought。输出应提供可复核的 Source cue、前提、中间判断、因果、条件分支、反例与边界；不需要输出内部草稿或不可验证的心理独白。
