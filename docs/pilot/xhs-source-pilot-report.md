...
| #3 | `6508552c000000001303f499` | #3 | `source-review` | 图片主导；有效 Raw image；OCR 保持 Derived；`month_day` |
...
来源发布时间是 2023-09-18，但 Raw 没有直接证明面试年份。
...
该样本同时有一张有效非空 Raw image。历史 `note_img_txt` 中虽然有详细问题列表，但仍只作为 OCR Derived。
...
### 1. Raw image fidelity review

Pilot #3 / #4 的原图 bytes 有效，但还没有完成逐图独立内容核对，因此仍处于 `source-review`。
...
不是因为根架构失败，而是因为 Pilot 正常发现了仍需验证的闭环。

进入全量迁移前至少还需要：

1. 对 Pilot #3 / #4 完成 Raw image fidelity review。
2. 至少把一个 Pilot 推进到 `source-ready`，证明 source lifecycle 能闭环。
3. 在一个 Pilot 上实际执行 source-first / no-lookahead 学习。
4. 实际重复运行 migration reconcile，证明幂等。
