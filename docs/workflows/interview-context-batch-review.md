...
Pilot request 的 `pilot_size` 最大为 50；#923 首批 request 应为恰好 50 条。每一条必须绑定 InterviewNote Issue number、body SHA-256 和 reviewed `InterviewContext`。
...
apply 前会重新读取依赖和全部 live Issues，逐条校验 body CAS；成功后按默认 1 秒间隔更新 title/labels，写入 `data/interview-contexts/`，并写 durable receipt comment。写入后重新读取并运行 InterviewNote validator。已有相同 batch、body digest、Context digest、title 和 labels 的 receipt 会被识别为 `already_applied`；只有投影完成但 receipt 缺失时补 receipt，不创建重复对象。

```bash
node scripts/plan-interview-context-learning-discovery.js \
  --request <request.md> --max-items 50 --rate-limit-ms 1000 --apply
```
