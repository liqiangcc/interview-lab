...
apply 前会重新读取依赖、四个 acceptance evidence、Issues、receipts 和全部 Git Context artifacts，并要求 re-check digest 与已确认 dry-run 完全一致；同时先以原子写入、fsync、rename 持久化逐项 apply intent/progress journal，启动时校验 batch、Issue、body/context/artifact/title/labels 映射。apply 进程还必须独占 progress lock；已有（包括 stale）lock 一律 fail closed，只有持有者在 `finally` 中释放。GitHub Issue GET 的 ETag 仅用于读取诊断；Issue PATCH 不发送 `If-Match`。锁内每个 candidate 在 PATCH 前都立即重读 body SHA、title 和完整 labels，并与前一次快照逐项比较；任何变化、validator 失败或 receipt 出现都停止/跳过并 fail closed。PATCH 使用完整 projection labels（保留所有当前非学习 labels，再替换 learning family），响应必须返回并精确匹配完整 labels；随后再次 live re-read exact projection 与 InterviewNote validator。由于 GitHub REST PATCH 没有受支持的 atomic precondition，GET→PATCH 之间仍存在 API TOCTOU 窗口；流程记录并接受该限制，不能声称这是并发 CAS。成功后按默认 1 秒间隔更新 title/labels 并写 receipt。脚本不会在 apply 中写本地 Context 文件：Context 必须在 mutation 前已经存在于可解析的 Git commit/ref，receipt 同时记录 artifact path/ref/commit/digest。Mutation 使用 `gh api --include`；HTTP 4xx/5xx 的 status、response body 与 GitHub request id 进入 durable failed error，mutation 不自动重试。receipt POST 前先原子持久化完整 receipt intent、receipt body digest 和 `receipt_pending/attempted=false`；紧接 POST 前再持久化 `attempted=true/possibly_performed=true`。因此进程若在 POST 后、响应前崩溃，恢复只在 apply lock 内 bounded live re-read receipt marker；marker 未出现、读取不确定或冲突时保持 fail closed，绝不盲目再次 POST。只有 durable state 明确为未发送时，才会使用同一 receipt intent 安全重试；#1509 等既有 failed progress 只保留为审计证据，后续恢复须在重新授权后使用新的 recovery progress，不得重置旧证据。每一条恢复重读路径都必须按 request 的 `audit_only_issue_numbers` 重新计算 scope；历史批次 receipt 不得因重读丢失 audit-only 语义而被误判为冲突或 repair。#3/#4/#915 的历史 receipt 只能在 receipt、artifact 与 projection 全部收敛时报告 `already_applied`，否则保持 fail closed。重跑同一 request 会先 live recheck 已成功项并跳过；failed 项只有 live 已明确收敛才能转 complete，不能盲目重发不确定 mutation。receipt 存在但 artifact 缺失/冲突、ref 不存在或 diverged、多 receipt 或 marker 冲突均 fail closed。

脚本不会修改 Raw Source、SourceNote body 或 InterviewNote machine record。Crash 后可用同一 request 重跑；body 漂移、identity/revision 漂移、依赖回退、validator 失败或 receipt 冲突都会停止，不自动猜测或覆盖。由于 Issue PATCH 缺少 endpoint-documented atomic CAS，立即快照检查只能缩小而不能消除外部并发修改的 TOCTOU 风险；因此每次 mutation 后的 exact projection/validator/readback 是必要条件，未收敛时保持 failed/fail-closed。
