...
SourceNote intake 完成后，`boundary:pending` 不允许手工绕过 machine record 直接创建 InterviewNote。正式 transition 使用 `source-note-boundary-review-transition.v1`（single/not-interview）或 `.v2`（multi-interview），详见 `docs/workflows/source-note-boundary-review.md`。

核心规则：body/state/SourceRevision 必须 CAS；`single-interview` identity 由 Source identity 自动推导；`multi-interview` 由 exact Source evidence 中稳定 `case_key` 派生 child identity，但只更新 SourceNote 的 Boundary Review 记录，仍须后续独立 materialization，不能把 SourceNote 直接当 InterviewNote。

## InterviewNote materialization

`boundary:single-interview` 只声明稳定 InterviewNote identity，不等于 InterviewNote Issue 已创建。正式物化必须走 `docs/workflows/source-note-interview-materialization.md` 的 CAS / ownership / validator / receipt 机制；禁止手工复制 SourceNote body 创建 InterviewNote。
